...
/* Called from isr_stubs.asm — common C entry point */
void isr_handler(struct isr_frame *frame) {
    ...
    /* Send EOI for hardware IRQs (ISR 32-47) */
    if (frame->int_no >= IRQ_BASE && frame->int_no < IRQ_BASE + 16) {
        pic_send_eoi(frame->int_no - IRQ_BASE);
    }
}
...
void idt_init(void) {
    ...
    /* CPU exceptions (ISR 0-31), selector 0x08 = kernel code, flags 0x8E = present, ring 0, 32-bit interrupt gate */
    idt_set_gate(0,  (uint32_t)isr0,  0x08, 0x8E);
    idt_set_gate(1,  (uint32_t)isr1,  0x08, 0x8E);
    idt_set_gate(2,  (uint32_t)isr2,  0x08, 0x8E);
    idt_set_gate(3,  (uint32_t)isr3,  0x08, 0xEE);  /* int3: DPL 3 for ring-3 debugger breakpoints */
    ...
    serial_log("IDT initialized (256 entries)");
}
